---
title: 鲜血死亡骑士减伤自愈与施法优先级
description: 鲜血死亡骑士骨盾维持、符文能量防溢出规则、大秘境聚怪接怪时序、死刑尖峰应对与减伤自疗优先级
...
1. **死刑读条前 2 秒**：停止泄能，预留至少 40 点符文能量（最好 80 点留备双灵打），确保身上骨盾层数在 7 层以上。
2. **死刑读条中**：根据伤害属性提前交对齐减伤：
   - 物理尖峰死刑：开启符文刃舞（招架）或吸血鬼之血（血上限与受愈倍增），并开启墓石消耗骨盾换取大额吸收盾。
   - 魔法尖峰死刑：提前 1 秒开启反魔法护罩（AMS）全额吸收或减免魔法穿透。
3. **死刑命中瞬间（0.5 秒内）**：立刻按下灵界打击，利用近期 5 秒高承伤机制瞬间回满血线，并生成巨额鲜血护盾抵挡后续物理补刀平砍。
